    ...
    <div class="overflow-x-auto mx-auto max-w-dvh">
        <div class="divide-x divide-zinc-700">
            @foreach ($posts as $post)
                ...
                @endphp
                {{-- FIX: onclick mal formado, faltaba window.location= --}}
                <div class="hover:bg-zinc-700/50 transition-colors cursor-pointer"
                    ...
                    <p class="px-6 py-4 whitespace-nowrap text-zinc-400">Autorea: {{ $user?->name ?? 'Ez dago' }}</p>
                    <p class="px-6 py-4 whitespace-nowrap text-zinc-400">{{ $post->created_at }}</p>
                    @if ($user && (($user->id === Auth::id() && $user->role === 'author') || $user->role === 'admin'))
