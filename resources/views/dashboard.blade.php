...
<div class="min-h-[80vh] flex items-center justify-center absolute z-10 top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 backdrop-blur-sm">
    ...
    {{-- Botón crear post + tabla de posts --}}
    <div class="mt-8">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-zinc-200">Posts</h3>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-zinc-700 text-zinc-200">
                            ...
                            <th class="px-6 py-4 text-left text-sm font-medium">Egilea</th>
                    ...
                    <tbody class="divide-y divide-zinc-700">
                        @foreach ($posts as $post)
                            <tr class="hover:bg-zinc-700/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-400">
                                    {{ \App\Models\User::find($post->user_id)?->name ?? 'Ezezaguna' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-zinc-400">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <form action="{{ route('posts.delete', $post->id) }}" method="POST" class="inline"
                                              onsubmit="return confirm('Post hau ezabatu nahi duzu?')">
                                            @csrf
                                            @method('DELETE')
